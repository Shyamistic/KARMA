...
export interface EvaluationResult {
  scores: {
    ...
    impact: number                // 0-25
    effort: number                // 0-25
    communityReception: number    // 0-25
  }
  totalScore: number              // 0-100
  tier: 'OUTSTANDING' | 'EXCELLENT' | 'GOOD' | 'EMERGING' | 'NO_TIP'
  amountUsdt: number              // 0, 1.00, 2.50, 5.00, 10.00
  reasoning: string               // Exactly 2 sentences
  sendTip: boolean
  skipReason?: string             // If sendTip=false, why
}
...
function assembleContext(profile: ContributionProfile): string {
...
async function runInference(context: string): Promise<string> {
  try {
...
function mapToToolAction(responseText: string): EvaluationResult {
  try {
    const result = JSON.parse(responseText.trim().replace(/```json|```/g, '')) as EvaluationResult
