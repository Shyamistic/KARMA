...
import { ContributionProfile, evaluateContribution } from '../../core/agent'
import { saveTip } from '../../lib/db'
import { createClaim, getClaimUrl } from './claims'
...
export async function getPrDetails(
  ...
  repo: string,
  prNumber: number
): Promise<ContributionProfile | null> {
  try {
    // Fetch PR data
    ...
    const approvals = reviews.filter((r: any) => r.state === 'APPROVED').length
    const rejections = reviews.filter((r: any) => r.state === 'CHANGES_REQUESTED').length
    ...
    const contributorPrCount = contributorPrs.filter(
      (p: any) => p.user?.login === pr.user?.login && p.merged_at
    ).length
    ...
    return {
      ...
      contributorPrCount,
      repoStars: (repoData as any).stargazers_count,
      repoName: `${owner}/${repo}`,
      isBot,
    }
  } catch (err) {
...
export async function postClaimComment(params: {
  owner: string
  repo: string
  prNumber: number
  contributor: string
...
function extractLinkedIssues(body: string): string[] {
  ...
  return matches.map((m: any) => m[1])
}

export async function processGithubPR(owner: string, repo: string, prNumber: number) {
  const profile = await getPrDetails(owner, repo, prNumber)
  if (!profile) return

  const evaluation = await evaluateContribution(profile)
  ...
  await postClaimComment({
    ...
    claimUrl: getClaimUrl(claim.claimToken),
